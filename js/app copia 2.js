...
function populateProfilePanel(ad) {
  ...
  if (ctaTitle) {
    ...
    } else if (available && service === "videocalls") {
  ...
  if (ctaBtnText) {
    ctaBtnText.textContent = service === "videocalls" ? "Videollamar" : "Llamar";
  }

  if (ctaIco) {
    ctaIco.innerHTML = service === "videocalls"
      ? buildVideoIconSvg()
      : buildPhoneIconSvg();
  }
}
...
(function initProfileModal(){
  ...
  // API para futuro (valores dinámicos)
  window.openProfileModal = open;
  window.closeProfileModal = close;
  window.setProfileModalData = function setProfileModalData(data = {}){
    if (data.code != null && elCode) elCode.textContent = String(data.code);
    if (data.phoneTail != null && elPhoneTail) elPhoneTail.textContent = String(data.phoneTail);
    if (data.tokens != null && elTokens) elTokens.textContent = String(data.tokens);
  };
  ...
  // Enlaces legales → abre el modal existente de políticas
  ...
  // Tienda de fichas (placeholder)
  storeBtn?.addEventListener("click", (e)=>{
    e.preventDefault();
    // futuro: abrir tienda
  });

  // ✅ Abandonar = cerrar logueo / salir
  logoutBtn?.addEventListener("click", (e)=>{
    ...
    // opcional: si quieres forzar que salga modal de login al abandonar:
    // window.openPhoneAuthModal?.();
  });
})();
