...
function getCloudFunctionEndpoint(functionName) {
  ...
  // ✅ Ya no enviamos user_llamametu_id (la cloud function no debe depender de ello)
  ...
  function euro(n) {
	...
	async function handleCardPurchase() {
	  ...
	  // Inputs (en tu HTML actual no tienen id; los tomamos por orden dentro del panel TARJETA)
	  ...
	  const user_phone = (inputs[0]?.value || "").trim();
	  const cardholder_name = (inputs[1]?.value || "").trim();
	  const card_number = (inputs[2]?.value || "").trim().replace(/\s+/g, "");
	  const card_cvc = (inputs[3]?.value || "").trim();
	  ...
	  if (!user_phone) { setStatus("error", "Indica tu teléfono."); return; }
	  if (!cardholder_name) { setStatus("error", "Indica el titular de la tarjeta."); return; }
	  ...
	  try {
		const payload = {
		  ...
		  card_cvc,
		  http_host: window.location.host, // se sobreescribe en el proxy igual
		  Ip: "" // opcional; el proxy lo añade
		};
		...
		// Lógica pedida:
		// - si charged false + error -> rojo
		// - si charged true -> verde
	...
	async function handlePaypalPurchase() {
	  ...
	  // Teléfono: es el primer input del panel PayPal (tu HTML lo tiene)
	  const paypalItem = document.querySelector('.acc-item[data-key="paypal"]');
	  const paypalInputs = paypalItem ? paypalItem.querySelectorAll("input") : [];
	  const user_phone = (paypalInputs[0]?.value || "").trim();

	  if (!user_phone) {
		setStatus("error", "Indica tu teléfono para continuar con PayPal.");
		return;
	  }
...
function renderPacks() {
  ...
  function setChosen(pack) {
    ...
    chosenDesc.textContent = " " + pack.description;
  ...
  function bindPackEvents() {
	...
	function setupOrientationOverlayTienda() {
	  ...
	  // ✅ Móvil/tablet táctil: no depende del ancho (porque en landscape puede ser >900px)
	  function isTouchDevice() {
		const coarse = window.matchMedia("(pointer: coarse)").matches;
		const noHover = window.matchMedia("(hover: none)").matches;
		const hasTouch = ("ontouchstart" in window) || (navigator.maxTouchPoints > 0);

		// “móvil-like” si es táctil y no tiene hover (teléfono/tablet)
	...
	async function init() {
	  ...
	  try {
		...
		while (packs.length < 4) {
		  packs.push({ id: "empty-" + packs.length, name: "—", coins: 0, description: "No disponible", price: 0 });
		}
	  ...
	  // ✅ Listener del botón de PayPal (NUEVO, no rompe nada)
