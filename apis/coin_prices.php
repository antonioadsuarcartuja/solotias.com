...
/**
 * CHICAS EN TU CIUDAD - Coin Prices API (SOLO LECTURA)
 *
 * Frontend -> /apis/coin_prices.php -> Back4App Cloud Function: api_get_coin_prices
 *
...
function parse_request(string $method, string $path, array $query = [], array $headers = [], ?string $bodyJson = null): array {
  ...
  if ($bodyJson !== null) curl_setopt($ch, CURLOPT_POSTFIELDS, $bodyJson);
...
// ---------- MAIN ----------
// Cloud Function (sin variables)
[$st, $json, $raw] = parse_request("POST", "/functions/api_get_coin_prices", [], [], "{}");
